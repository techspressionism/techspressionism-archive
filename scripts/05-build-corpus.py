...
from lib_corrections import apply_style_rules, apply_vocabulary, EMPHASIS_WORDS, collapse_stutters, correct_text, fix_doubled_punctuation, fix_midsentence_caps, _ordinary_words
...
def build_known_terms(session, vocab_terms, artists):
    ...
    for t in vocab_terms:
        if not t.get("variants_only"):       # a plain word ("Loop") must not capitalise every "loop"; only its listed mishearings change
...
def process_session(session, artists, vocab_terms, review_rows):
    ...
    for seg in segments:
        seg["text"] = fix_midsentence_caps(apply_style_rules(italicize_titles(fix_spoken_decades(seg["text"].replace("&nbsp;", " ")))))
        if seg["speaker"]:
    ...
    for seg in segments:            # doubled punctuation left behind by the steps above (".." / ",,"): the last text change before sentence times are worked out
        seg["text"] = fix_doubled_punctuation(collapse_stutters(seg["text"], EMPHASIS_WORDS))
