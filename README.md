# Techspressionism Video Archive
...
| 4 — Name correction | Run across all 99 sessions | `scripts/04-correct-names.py` + `scripts/04a-parse-artist-index.py`. ~940 auto-corrections applied; `review/name-candidates.csv` is the raw hit list, **`review/name-candidates-summary.csv`** collapses it to ~250 unique patterns to actually work from (run `scripts/dedupe-review-queue.py` after a rebuild). `data/review-suppressions.json` = hand-curated always-false-positive pairs; recurring real manglings promoted into `data/vocabulary.json`. Note: `data/artists.json` was hand-cleaned of location/nickname junk in names -- re-running `04a` needs a fresh WordPress export (the one used is no longer on disk) and its parser fix will reproduce the clean output. |
...
- **Transcript source priority: Zoom transcript first, YouTube captions as fallback.** Not what the original spec assumed (captions-first). Reasoning: Zoom's transcript is attributed per utterance (turn-by-turn, including Q&A/discussion), where a YouTube caption track is one undifferentiated stream that only Stage 1's manual speaker index can (partially) segment -- and that index only marks where each person's *presentation* starts, not discussion. Confirmed via a same-video duration check (local Zoom recording vs. published YouTube video, 3 sessions spanning 2021-2026) that Colin uploads the raw, untrimmed Zoom recording, so Zoom transcript timestamps map directly onto YouTube timestamps for deep-linking. **Correction (Sept 2026):** this held for the 3 sessions checked but not in general -- some Salons are edited (sections cut, start trimmed) before upload, and roundtables get an intro added. All timecodes must be on the YouTube video's timeline; `scripts/02b-align-zoom-timeline.py` and `02c-find-cut-material.py` convert Zoom timing (manual, section 6.4).
